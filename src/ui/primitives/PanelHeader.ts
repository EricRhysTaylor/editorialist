...
export function renderPanelHeader(parent: HTMLElement, plugin: EditorialistPlugin, viewType: string, label: string): HTMLElement {
	...
	const launch = row.createEl("button", { text: "Import", attr: { type: "button", "aria-label": "Import revision notes" } });
	...
	header.createDiv({ cls: "editorialist-panel__book", text: plugin.getActiveBookScopeInfo().label ?? "No active book selected" });
	return header;
}
